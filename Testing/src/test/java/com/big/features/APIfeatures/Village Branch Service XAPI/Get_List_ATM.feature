@ignore
Feature: Village Branch | Get List ATM
Background:
  ...
  Scenario: 4 Check for response headers and its values and compare same with layer 7 - [Content-Length]
    ...
    * def ContentLength_L7 = Expected_headers['Get_List_ATM']['Response Headers']['content-length']
  ...
  Scenario: 6 Check for responseTime and compare with expected response time - [ResponseTime]
    ...
    * assert responseTime <= ExpectedResponseTime
    ...
    # 7 Valid BranchCity
  Scenario: 7 Check for response and compare same with layer 7 for valid parameter -[BranchCity]
    Given url Baseurl + subpath + BranchCity
    And header Content-Type = ContentType
    And header Authorization = 'Bearer '+ token
    When method GET
    ...
    * match L7_response_Branch == MuleResponse_Branch 
    ...
    # 8a Invalid BranchCity
  Scenario: 8a Check for response and compare same with layer 7 for invalid parameter - [InvalidBranchCity]
