...
Feature: CC XAPI | Get PAN Transaction
Background:
  ...
  Scenario: 5 Check for responseTime and compare with expected response time - [ResponseTime]
    ...
    * assert responseTime <= ExpectedResponseTime
    ...
    # 6 Valid PAN
  Scenario: 6 Check for response and compare same with layer 7 for valid parameter -[PAN]
    Given url Baseurl + subpath + PAN + "/transactions"
    And header Content-Type = ContentType
    And header Authorization = 'Bearer '+ token
    When method GET
    ...
    * match L7_response_data == Response_mule   
    ...
    # 7 invalid PAN
  Scenario: 7 Check for response and compare same with layer 7 for invalid parameter - [invalid PAN]
    ...
    * def L7Response_ApiVersionNumber = L7_InvalidResponse['GetPanTransaction']['ApiVersionNumber']
    * def L7Response_IsSuccess = L7_InvalidResponse['GetPanTransaction']['IsSuccess']
