...
Background:
  ...
  Scenario: 4 Check for response headers and its values and compare same with layer 7 - [Content-Length]
    ...
    * def ContentLength_L7 = Expected_headers['Get_ATM_by_ID']['Response Headers']['content-length']
  ...
  Scenario: 6 Check for responseTime and compare with expected response time - [ResponseTime]
    ...
    * assert responseTime <= ExpectedResponseTime
    ...
    # 7 Valid ValidATMID
  Scenario: 7 Check for response and compare same with layer 7 for valid parameter -[ValidATMID]
    Given url Baseurl + subpath + ValidATMID
    And header Content-Type = ContentType
    And header Authorization = 'Bearer '+ token
    When method GET
    ...
    * match L7_response_ATMServices == MuleResponse_ATMServices 
    ...
    # 8a Invalid ATM ID
  Scenario: 8a Check for response and compare same with layer 7 for invalid parameter - [InvalidATMID]
    ...
    * match L7Response_Message == MuleResponse_Message
    ...
    # 8b No ATM ID
  Scenario: 8b Check for response and compare same with layer 7 for invalid parameter - [NoATMID]
