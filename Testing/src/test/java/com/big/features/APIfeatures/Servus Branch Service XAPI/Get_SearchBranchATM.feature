...
Feature: Servus Branch | Search Branch ATM
Background:
  ...
  Scenario: 5 Check for responseTime and compare with expected response time - [ResponseTime]
    ...
    * assert responseTime <= ExpectedResponseTime
    ...
    # 6 Valid BranchName
  Scenario: 6 Check for response and compare same with layer 7 for valid parameter -[BranchName]
    Given url Baseurl + subpath + BranchName
    And header Content-Type = ContentType
    And header Authorization = 'Bearer '+ token
    When method GET
    ...
    * match L7_response_Name == MuleResponse_Name    
    ...
    # 7 InvalidBranchName
  Scenario: 7 Check for response and compare same with layer 7 for invalid parameter - [InvalidBranchName]
