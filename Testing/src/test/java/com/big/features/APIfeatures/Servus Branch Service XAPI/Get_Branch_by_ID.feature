...
Feature: Servus Branch | Get Branch by ID
Background:
  ...
  Scenario: 5 Check for responseTime and compare with expected response time - [ResponseTime]
    ...
    * assert responseTime <= ExpectedResponseTime
    ...
    # 6 Valid ValidBranchID
  Scenario: 6 Check for response and compare same with layer 7 for valid parameter -[ValidBranchID]
    Given url Baseurl + subpath + ValidBranchID
    And header Content-Type = ContentType
    And header Authorization = 'Bearer '+ token
    When method GET
